...
import { DollarSign, TrendingUp, Users, Target, Trophy, Calendar, Receipt } from 'lucide-react'
...
export default function MonthlyInvoicingOverview() {
    ...
    async function fetchData() {
      ...
      try {
        const [salesRes, customersRes, summaryRes] = await Promise.all([
          fetch(`${API_BASE_URL}/api/invoices-leaderboard-sales-this-month`).then(r => r.json()),
          fetch(`${API_BASE_URL}/api/top-customers-invoicing-this-month`).then(r => r.json()),
        ...
        setInvoiceCount(summaryRes.invoice_count || 0)
      } catch (error) {
        console.error('Failed to fetch monthly invoicing overview:', error)
      } finally {
        setLoading(false)
      }
    }
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {invoiceCount > 0 ? formatCurrency(totalInvoicing / invoiceCount) : '$0'}
                </p>
          ...
          <CardContent className="p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">with invoices</p>
              </div>
            </div>
          </CardContent>
        </Card>
                ...
                <div key={person.salesperson_name} className="flex items-center justify-between p-3 rounded-lg bg-gray-50">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500">{person.invoice_count} invoices</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="font-semibold text-gray-900">{formatCurrency(person.total_invoiced)}</p>
