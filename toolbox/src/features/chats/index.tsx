...
export default function Chats() {
  ...
  const sortedMessages = selectedUser ? [...selectedUser.messages].sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime()) : []
  const currentMessage = sortedMessages.reduce(
  ...
  // Handler for renaming a message
  async function handleRenameMessage(e: React.FormEvent, messageTimestamp: string) {
    e.preventDefault();
    if (!selectedUser) return;
    const chatId = selectedUser.id;
    await fetch(`/api/rfp-gpt/chats/${chatId}/messages/${messageTimestamp}`, {
      method: 'PATCH',
    ...
    updated.messages = updated.messages.map((msg) =>
      msg.timestamp === messageTimestamp ? { ...msg, message: editContent } : msg
    );
  ...
  async function handleDeleteMessage(messageTimestamp: string) {
    if (!selectedUser) return;
    ...
    // Update local state
    const updated = { ...selectedUser };
    updated.messages = updated.messages.filter((msg) => msg.timestamp !== messageTimestamp);
    setSelectedUser(updated);
  }

  // Handler for renaming a chat
  async function handleRenameChat(e: React.FormEvent, chatId: string) {
    e.preventDefault();
    await fetch(`/api/rfp-gpt/chats/${chatId}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
    ...
    if (idx !== -1) {
      conversations[idx].fullName = editChatName;
    }
  ...
  async function handleDeleteChat(chatId: string) {
    ...
    await fetch(`/api/rfp-gpt/chats/${chatId}`, { method: 'DELETE' });
    // Update local state
    const idx = conversations.findIndex((c) => c.id === chatId);
    if (idx !== -1) {
      conversations.splice(idx, 1);
      if (selectedUser?.id === chatId) setSelectedUser(null);
    }
  }
                ...
                return (
                  <Fragment key={id}>
                    ...
                    <button
                      type='button'
                      className={cn(
                          `hover:bg-secondary/75 -mx-1 flex-1 rounded-md px-2 py-2 text-left text-sm`,
                    ...
                    >
                      <div className='flex gap-2'>
                        ...
                        <div>
                            {editingChatId === id ? (
                              <form onSubmit={e => handleRenameChat(e, id)} className='flex gap-1'>
                                <input
                                  value={editChatName}
                                  onChange={e => setEditChatName(e.target.value)}
                                  className='w-24 rounded border px-1 text-sm'
                                  autoFocus
                                />
                                <Button type='submit' size='sm'>Save</Button>
                                <Button type='button' size='sm' variant='ghost' onClick={() => setEditingChatId(null)}>Cancel</Button>
                              </form>
                            ) : (
                            ...
                            {currentMessage[key].map((msg, index) => (
                              ...
                              >
                                {editingMsgId === msg.timestamp ? (
                                  <form onSubmit={e => handleRenameMessage(e, msg.timestamp)} className="flex flex-col gap-1">
                                    <input
                                      value={editContent}
                                    ...
                                    <div className="flex gap-2 mt-1">
                                      <Button type="submit" size="sm">Save</Button>
                                      <Button type="button" size="sm" variant="ghost" onClick={() => setEditingMsgId(null)}>Cancel</Button>
                                    </div>
                                  </form>
                                ) : (
                                  <>
